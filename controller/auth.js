...
const token = async (req, res) => {
  ...
  const access = jwt.sign(user_details, process.env.TOKEN_SECRET, {
    expiresIn: "15m",
  });
  const refresh = jwt.sign(user_details, process.env.REFRESH_TOKEN_SECRET, {
    expiresIn: "30d",
  });
...
const token_refresh = async (req, res) => {
  ...
  try {
    ...
    if (user.refresh === refresh) {
      const new_token = jwt.sign(user_details, process.env.TOKEN_SECRET, {
        expiresIn: "15m",
      });
      ...
      User.updateOne(
        ...
        (err, docs) => {
          if (err) return res.sendStatus(403);
            ...
            return res
              .status(200)
              .json({ access: new_token, refresh: new_refresh });
        }
      );
